...
{
    /// <summary>
    ///     A <see cref="System.Diagnostics.TraceListener" /> that writes to a database, formatting the output with an
    ...
    {
        private readonly string _addCategoryStoredProcName = String.Empty;
        private readonly Database _database;
        private readonly string _writeLogStoredProcName = String.Empty;
        ...
        public override void Write(string message)
        {
            using (var transaction = new TransactionScope(TransactionScopeOption.RequiresNew))
            {
                ExecuteWriteLogStoredProcedure(0, 5, TraceEventType.Information, string.Empty, DateTime.Now, string.Empty,
                    string.Empty, string.Empty, string.Empty, null, null, message, _database);
        ...
        {
            return new []
            {"formatter", "writeLogStoredProcName", "addCategoryStoredProcName", "databaseInstanceName"};
        }
        ...
        private bool ValidateParameters()
        {
            ...
            return !string.IsNullOrEmpty(_addCategoryStoredProcName);
        }
        ...
        private void ExecuteWriteLogStoredProcedure(int eventId, int priority, TraceEventType severity, string title, DateTime timeStamp, string machineName, string appDomainName, string processId, string processName, string managedThreadName, string win32ThreadId, string message, Database db)
        {
            using (var cmd = db.GetStoredProcCommand(_writeLogStoredProcName))
            {
                ...
                db.AddParameter(cmd, "machineName", DbType.String, 32, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, machineName);
                db.AddParameter(cmd, "AppDomainName", DbType.String, 512, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, appDomainName);
                db.AddParameter(cmd, "ProcessID", DbType.String, 256, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, processId);
                db.AddParameter(cmd, "ProcessName", DbType.String, 512, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, processName);
                db.AddParameter(cmd, "ThreadName", DbType.String, 512, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, managedThreadName);
                db.AddParameter(cmd, "Win32ThreadId", DbType.String, 128, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, win32ThreadId);
                db.AddParameter(cmd, "message", DbType.String, 1500, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, message);
                db.AddInParameter(cmd, "formattedmessage", DbType.String, message);

                db.AddOutParameter(cmd, "LogId", DbType.Int32, 4);

                db.ExecuteNonQuery(cmd);
                //var logId = Convert.ToInt32(cmd.Parameters[cmd.Parameters.Count - 1].Value, CultureInfo.InvariantCulture);
            }
        }
        ...
        /// <param name="db">An instance of the database class to use for storing the LogEntry</param>
        /// <returns>An integer for the LogEntry Id</returns>
        private int ExecuteWriteLogStoredProcedure(LogEntry logEntry, Database db)
        {
            using (var cmd = db.GetStoredProcCommand(_writeLogStoredProcName))
            {
                var logEntryEx = logEntry as LogEntryEx;
                ...
                db.AddParameter(cmd, "AppDomainName", DbType.String, 512, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, logEntry.AppDomainName);
                db.AddParameter(cmd, "ProcessID", DbType.String, 256, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, logEntry.ProcessId);
                db.AddParameter(cmd, "ProcessName", DbType.String, 512, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, logEntry.ProcessName);
                db.AddParameter(cmd, "ThreadName", DbType.String, 512, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, logEntry.ManagedThreadName);
                db.AddParameter(cmd, "Win32ThreadId", DbType.String, 128, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, logEntry.Win32ThreadId);
                db.AddParameter(cmd, "message", DbType.String, 1500, ParameterDirection.Input, false, 0, 0, null,
                    DataRowVersion.Default, (logEntryEx ?? logEntry).Message);

                db.AddInParameter(cmd, "formattedmessage", DbType.String, Formatter != null ? Formatter.Format(logEntry) : logEntry.Message);

                db.AddParameter(cmd, @"jsonObject", DbType.String, 1500, ParameterDirection.Input, false, 0, 0, null, DataRowVersion.Default, logEntryEx != null ? logEntryEx.JsonObject : null);
        ...
        /// <param name="db">An instance of the database class to use for storing the LogEntry</param>
        private void ExecuteAddCategoryStoredProcedure(LogEntry logEntry, int logID, Database db)
        {
            foreach (var category in logEntry.Categories)
            {
                var cats = category.Split(' ');
                var cat =  cats.Length > 0 ? cats[0] : category;
                var actionId =  cats.Length > 1 ? cats[1]: logEntry.ActivityIdString;

                using (var cmd = db.GetStoredProcCommand(_addCategoryStoredProcName))
                {
                    db.AddParameter(cmd, @"categoryName", DbType.String, 64, ParameterDirection.Input, false, 0, 0, null, DataRowVersion.Default, cat);
                    db.AddInParameter(cmd, @"logID", DbType.Int32, logID);
                    db.AddInParameter(cmd, @"actionID", DbType.Guid, string.IsNullOrEmpty(actionId) ? Guid.Empty : Guid.Parse(actionId));
